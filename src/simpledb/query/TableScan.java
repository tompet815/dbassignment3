...
public class TableScan implements UpdateScan {
    ...
    public Constant getVal(String fldname) {
        if (sch.type(fldname) == INTEGER) {
            ...
            return new FloatConstant(Float.valueOf(rf.getString(fldname)));
        }
        else if (sch.type(fldname) == BOOLEAN) {
            return new BoolConstant((Boolean) (rf.getInt(fldname) == 1));
        }
        else {
    ...
    public void setVal(String fldname, Constant val) {
        if (sch.type(fldname) == INTEGER) {
            ...
            int boolInt = 0;//false
            if ((Boolean) val.asJavaVal()) {
                boolInt = 1;
            }
            rf.setInt(fldname, boolInt);
        }
        else if (sch.type(fldname) == FLOAT) {
            rf.setString(fldname, val.asJavaVal().toString());
        }
        else {
